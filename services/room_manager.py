...
class MonitoredRoom:
    ...
    def __init__(self, live_id: str, room_id: int, db_session, manager, socketio=None):
        """
        初始化监控房间
        :param live_id: 直播间ID
        :param room_id: 数据库中的房间ID
        ...
        logger.info(f"创建监控房间实例: live_id={live_id}, room_id={room_id}")
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if not is_live:
                    ...
                    # 根据监控类型决定下一步操作
                    room = self.manager.data_service.get_live_room(self.room_id)
                    if room and room.monitor_type == '24h' and room.auto_reconnect:
                        ...
                        if self._poll_room_status():
                            ...
                            self.manager.data_service.update_live_room_reconnect(self.room_id, 0)
                            continue
                        else:
                            # 轮询超时或被中断
                            break
...
class RoomManager:
    ...
    def __init__(self, data_service: DataService, socketio=None):
        ...
        self.socketio = socketio
        self.active_rooms: Dict[int, MonitoredRoom] = {}  # room_id -> MonitoredRoom
        ...
        try:
            ...
            for room in monitoring_rooms:
                ...
                if room.id not in self.active_rooms:
                    logger.warning(f"房间 {room.live_id} (id={room.id}) 状态为 monitoring 但实际未监控，重置状态")
    ...
    def add_room(self, live_id: str, monitor_type: str = 'manual', auto_reconnect: bool = False) -> Optional[int]:
        """
        添加监控房间
        :param live_id: 直播间ID
        :param monitor_type: 监控类型 (24h/manual)
        :param auto_reconnect: 是否自动重连
        :return: 房间ID，失败返回None
        """
        with self.lock:
            # 检查是否已存在
            existing_room = self.data_service.get_live_room_by_live_id(live_id)
            if existing_room:
                if existing_room.id in self.active_rooms:
                    logger.warning(f"直播间 {live_id} 已在监控中")
                    return None
                room_id = existing_room.id
            else:
                # 创建新房间记录
                room = self.data_service.create_live_room(
                    live_id=live_id,
            ...
            self.active_rooms[room_id] = monitored_room
            logger.info(f"添加监控房间: live_id={live_id}, room_id={room_id}")
            return room_id
    ...
    def remove_room(self, room_id: int) -> bool:
        ...
        with self.lock:
            if room_id not in self.active_rooms:
                logger.warning(f"房间 {room_id} 不在活跃列表中")
                return False

            monitored_room = self.active_rooms[room_id]
            monitored_room.stop()
            del self.active_rooms[room_id]
            logger.info(f"移除监控房间: room_id={room_id}")
            return True

    def get_room(self, room_id: int) -> Optional[MonitoredRoom]:
        """获取监控房间实例"""
        return self.active_rooms.get(room_id)

    def get_room_by_live_id(self, live_id: str) -> Optional[MonitoredRoom]:
        """根据live_id获取监控房间实例"""
        room = self.data_service.get_live_room_by_live_id(live_id)
        if room:
            return self.active_rooms.get(room.id)
        return None
    ...
    def start_room(self, room_id: int) -> bool:
        ...
        with self.lock:
            ...
            if not monitored_room:
                ...
                self.active_rooms[room_id] = monitored_room
                logger.info(f"重新创建监控房间实例: room_id={room_id}")

            monitored_room.start()
            return True

    def stop_room(self, room_id: int) -> bool:
        """
        停止房间监控
        :param room_id: 房间ID
        :return: 是否成功
        ...
        with self.lock:
            ...
            if not monitored_room:
                ...
                if room and room.status == 'monitoring':
                    logger.warning(f"房间 {room_id} 不在活跃列表中但数据库状态为 monitoring，重置状态")
                    self.data_service.update_live_room_status(
                        room_id,
                        'stopped',
                        '状态不一致，已重置'
    ...
    def restart_failed_rooms(self) -> int:
        ...
        with self.lock:
            ...
            for room_id, monitored_room in list(self.active_rooms.items()):
                # 检查是否需要重启
                room = self.data_service.get_live_room(room_id)
                if room and room.status in ('error', 'stopped') and room.auto_reconnect:
                    ...
                    if not monitored_room.thread or not monitored_room.thread.is_alive():
                        rooms_to_restart.append(room_id)

            for room_id in rooms_to_restart:
                monitored_room = self.active_rooms.get(room_id)
                if monitored_room:
                    monitored_room.start()
                    restarted += 1
                    logger.info(f"重启失败的房间: room_id={room_id}")
    ...
    def get_all_rooms_status(self) -> list:
        ...
        with self.lock:
            status_list = []
            for room_id, monitored_room in self.active_rooms.items():
                room = self.data_service.get_live_room(room_id)
                if room:
                    status_list.append({
                        'id': room.id,
        ...
        with self.lock:
            for room_id, monitored_room in list(self.active_rooms.items()):
                try:
                    ...
                    logger.error(f"关闭房间 {room_id} 时出错: {e}")
